...
function sendMembership(req, res) {
    var dataToSend = req.body;

    dataToSend.memberSinceDate = utils.getDateNow();
    dataToSend.memberUntilDate = utils.getDateNextYear();
    dataToSend.bodycode = req.bodycode;
    dataToSend.userid = req.body.uid;
    dataToSend.bodyName= req.body.givenName;

    //TODO where to get these values??
    dataToSend.bodyCategory = '';
    dataToSend.bodyCode = '';
    ...
    //API http://docs.omscore.apiary.io/#reference/users/create-new-application-to-membership/create-new-application-to-membership
    //no text? where to store the motivation letter? why need
    options.path = '/users/'+dataToSend.uid+'/memberships/create';

    restClient.post(options, function(resterr, restreq, restres, restobj) {
        message = { 'outcome': 'success', 'message': 'Confirmation woot woot'}
...
function renderApplymembership(req, res){
    var applicaton = true;
...
router.post('/members/applymembership',isLoggedIn, sendMembership);
